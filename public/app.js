...
socket.on('player_private_update', ({ player, assignedRequests, myRequest }) => {
  ...
  if (player.role === 'PENDING') {
    ...
    if (player.role === 'ÁPORO') {
      btnPaySurvival.textContent = 'Pedir Ayuda (Expone tu Dignidad)';
      if (survivalDescElement) {
        survivalDescElement.textContent = 'No tienes recursos. Depender de la caridad del sistema o de los demás te expone al rechazo, lo que puede reducir tu dignidad según la ética de Adela Cortina.';
      }
    } else {
